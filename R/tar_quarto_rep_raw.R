...
tar_quarto_rep_run <- function(args, execute_params, deps) {
  assert_quarto()
  rm(deps)
  gc()
  execute_params <- split(execute_params, f = seq_len(nrow(execute_params)))
  unname(unlist(map(execute_params, ~tar_quarto_rep_rep(args, .x))))
}
...
tar_quarto_rep_rep <- function(args, execute_params) {
  ...
  sort(as.character(fs::path_rel(unlist(c(args$input, args$output_file)))))
}
